...
namespace NPC.UtilityAI.Considerations
{
    ...
    public class NotCloseToPlayer : Consideration
    {
        ...
        /// This method calculates the distance between the npc's position and the player's position using Vector3.Distance and returns a score that is inversely proportional to the distance and normalized by the npc's sight range. The score is also adjusted by subtracting 1.3f from 1 before dividing by the sight range. If the player field is null, it returns 0 as the score.
        /// </remarks>
        /// <param name="npc">
        ...
        public override float ScoreConsideration(NonPlayerCharacter npc)
        {
            ...
            // Return a score that is inversely proportional to the distance and normalized by the npc's sight range. The score is also adjusted by subtracting 1.3f from 1 before dividing by the sight range using 1 - (1.3f - (distance / npc.stats.sightRange)).
